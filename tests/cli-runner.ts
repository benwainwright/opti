...
export const runCli = async (args: string[], options: CliRunnerOptions = {}): Promise<CliResult> => {
...
async function runCliSubprocess(args: string[], options: { timeout: number; cwd: string }): Promise<CliResult> {
  try {
    ...
    return {
      ...
      stderr: (error.stderr || error.message || '').split('\n').filter((line: string) => line.trim()),
...
async function runCliInProcess(args: string[]): Promise<CliResult> {
  ...
  console.log = vi.fn((...args) => {
    stdout.push(args.map(arg => typeof arg === 'string' ? arg : JSON.stringify(arg)).join(' '));
  });

  console.error = vi.fn((...args) => {
    stderr.push(args.map(arg => typeof arg === 'string' ? arg : JSON.stringify(arg)).join(' '));
  });
