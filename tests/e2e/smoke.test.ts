...
describe("E2E Smoke Tests (Real API)", () => {
  ...
  beforeAll(() => {
    if (!apiToken) {
      console.warn(
        "⚠️  OPTIMIZELY_API_TOKEN not found. E2E smoke tests will be skipped.",
      );
      console.warn(
        "   To run these tests, set the OPTIMIZELY_API_TOKEN environment variable.",
      );
      console.warn(
        "   You can create a .env file in the project root with: OPTIMIZELY_API_TOKEN=your_token_here",
      );
    }
  });
  ...
  describe("Real API Integration", () => {
    ...
    it.skipIf(!apiToken)(
      "should list flags with valid token and project (may fail if no project exists)",
      async () => {
    ...
    it.skipIf(!apiToken)("should get flag details", async () => {
      ...
      if (!testProjectId || !testFlagKey) {
        console.warn(
          "⚠️  Skipping flag details test - no project or flag available",
        );
        return;
      }
      const result = await runCli(
    ...
    it.skipIf(!apiToken)(
      "should get flag ruleset (may fail if environment does not exist)",
      async () => {
        const testProjectId = envProjectId || projectId;
        const testFlagKey = envFlagId || flagKey;
        ...
        if (!testProjectId || !testFlagKey) {
          console.warn(
            "⚠️  Skipping ruleset test - no project or flag available",
          );
          return;
        }
        const result = await runCli(
        ...
        if (result.success) {
          ...
          console.log(
            `✅ Successfully retrieved ruleset for flag: ${testFlagKey}`,
          );
        } else {
          console.warn(
            `⚠️  Could not get ruleset for flag ${testFlagKey} in environment ${environmentKey}.`,
          );
          console.warn(
            `   This is acceptable if the environment doesn't exist.`,
          );
  ...
  describe("Error Handling", () => {
    it.skipIf(!apiToken)(
      ...
      async () => {
        ...
        expect(result.exitCode).not.toBe(0);
        expect(result.stderr.join("\n").toLowerCase()).toMatch(
          /404|403|not found|forbidden|project/i,
        );
    ...
    it.skipIf(!apiToken)(
      "should handle non-existent flag gracefully",
      async () => {
        const testProjectId = envProjectId || projectId;

        if (!testProjectId) {
          console.warn(
            "⚠️  Skipping non-existent flag test - no project available",
          );
          return;
        }
        ...
        const result = await runCli(
          [
            ...
            testProjectId,
            "--flag_key",
            "definitely-nonexistent-flag-12345",
            "--token",
            apiToken!,
            "--renderer",
            "json",
  ...
  if (!projectId && !envProjectId) {
    console.warn(
      "⚠️  E2E smoke tests completed but no valid project was found.",
    );
    console.warn(
      "   This is expected if you don't have any projects in your Optimizely account.",
    );
    console.warn(
      "   Create a project and some flags to enable full E2E testing.",
    );
  } else {
    console.log(
      `✅ E2E smoke tests completed successfully with project: ${envProjectId || projectId}`,
    );
